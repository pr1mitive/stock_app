...
(function() {
  ...
  console.log('[INVENTORY] Projection Script loaded - Version: 2.0');
  ...
  async function updateInventoryProjection(itemCode, warehouse, location) {
    try {
      ...
      // 過去30日分の期首在庫を計算（逆算）
      const openingQtyMap = calculateOpeningQuantities(
        dailyTransactions,
        currentQty,
        startDate,
        today
      ...
      // 各日付のサマリーを作成・更新
      const summaryData = [];
      let runningQty = openingQtyMap.get(Utils.formatDate(startDate)) || 0;

      for (let i = 0; i <= PAST_DAYS + FUTURE_DAYS; i++) {
        const targetDate = Utils.addDays(startDate, i);
        ...
        // 期首在庫を設定
        const openingQty = openingQtyMap.get(dateStr) || runningQty;
   ...
   * 過去の期首在庫を逆算
   */
  function calculateOpeningQuantities(dailyTransactions, currentQty, startDate, today) {
    const openingMap = new Map();
    
    // 今日の期首在庫を計算（今日の取引を逆算）
    const todayStr = Utils.formatDate(today);
    const todayData = dailyTransactions.get(todayStr) || { received_qty: 0, issued_qty: 0 };
    const todayOpening = currentQty - todayData.received_qty + todayData.issued_qty;
    
    openingMap.set(todayStr, todayOpening);

    // 過去に遡って期首在庫を計算
    let runningQty = todayOpening;
    
    for (let i = 0; i < PAST_DAYS; i++) {
      const targetDate = Utils.addDays(today, -(i + 1));
      const dateStr = Utils.formatDate(targetDate);
      const dailyData = dailyTransactions.get(dateStr) || { received_qty: 0, issued_qty: 0 };
      
      // 前日の期首在庫 = 当日の期首在庫 - 前日の入庫 + 前日の出庫
      runningQty = runningQty - dailyData.received_qty + dailyData.issued_qty;
      openingMap.set(dateStr, runningQty);
    }

    // 未来の期首在庫は当日の期末在庫をベースに計算
    let futureRunning = currentQty;
    for (let i = 1; i <= FUTURE_DAYS; i++) {
      const targetDate = Utils.addDays(today, i);
      const prevDate = Utils.addDays(today, i - 1);
      const prevDateStr = Utils.formatDate(prevDate);
      const prevDailyData = dailyTransactions.get(prevDateStr) || { received_qty: 0, issued_qty: 0 };
      
      // 前日の期末在庫 = 前日の期首 + 前日の入庫 - 前日の出庫
      futureRunning = futureRunning + prevDailyData.received_qty - prevDailyData.issued_qty;
      openingMap.set(Utils.formatDate(targetDate), futureRunning);
    }
