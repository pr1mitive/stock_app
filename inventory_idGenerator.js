...
(function(PLUGIN_ID) {
  ...
  /**
   * balance_id を生成
   * 形式: BAL-{item_code}-{warehouse}-{location}
   * 例: BAL-A001-TKY-A01
   * 
   * ※ レコード作成前に実行
   */
  kintone.events.on('app.record.create.submit', (event) => {
    ...
    try {
      ...
      const warehouse = UTILS.getFieldValue(event.record, FIELDS.BALANCE.WAREHOUSE);
      ...
      // 倉庫コードとロケーションコードを短縮形に変換（省略可能）
      const warehouseCode = convertWarehouseCode(warehouse);
      const locationCode = convertLocationCode(location);
  ...
  /**
   * summary_id を生成
   * 形式: SUM-{item_code}-{warehouse}-{location}-YYYYMMDD
  ...
  kintone.events.on('app.record.create.submit', (event) => {
    ...
    try {
      ...
      const warehouse = UTILS.getFieldValue(event.record, FIELDS.PROJECTION.WAREHOUSE);
  ...
  /**
   * 倉庫名を短縮コードに変換
   * @param {string} warehouse - 倉庫名
   * @returns {string} 倉庫コード
   */
  function convertWarehouseCode(warehouse) {
    // カスタマイズ可能：倉庫名 → コード変換
    const warehouseMap = {
      '東京倉庫': 'TKY',
      '大阪倉庫': 'OSK',
    ...
    return warehouseMap[warehouse] || warehouse.substring(0, 3).toUpperCase();
  }

  /**
   * ロケーション名を短縮コードに変換
   * @param {string} location - ロケーション名
   * @returns {string} ロケーションコード
   */
  function convertLocationCode(location) {
    // そのまま返す（例: A-01, B-15）
    // 必要に応じてカスタマイズ可能
    return location;
  }
  ...
  window.INVENTORY_ID_GENERATOR = {
    convertWarehouseCode: convertWarehouseCode,
    convertLocationCode: convertLocationCode,
