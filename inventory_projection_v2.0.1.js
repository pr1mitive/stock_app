...
(function() {
  ...
  console.log('[INVENTORY] Projection Script loaded - Version: 2.0.1');
  ...
  async function updateInventoryProjection(itemCode, warehouse, location) {
    try {
      ...
      // 在庫取引を取得（開始日の前日〜未来90日）
      // 前日から取得することで、開始日の期首在庫を正確に計算できる
      const queryStartDate = Utils.addDays(startDate, -1);
      const startDateStr = Utils.formatDate(queryStartDate);
      ...
      if (todayData) {
        Utils.log(`今日(${todayStr})の取引:`, todayData);
      }

      // 今日の期首在庫を逆算
      // 現在在庫 = 今日の期首 + 今日の確定入庫 - 今日の確定出庫
      // → 今日の期首 = 現在在庫 - 今日の確定入庫 + 今日の確定出庫
      const todayTx = dailyTransactions.get(todayStr) || { received_qty: 0, issued_qty: 0 };
      const todayOpening = currentQty - todayTx.received_qty + todayTx.issued_qty;

      Utils.log(`今日の期首在庫（逆算）: ${todayOpening}`);
      Utils.log(`今日の入庫: ${todayTx.received_qty}, 出庫: ${todayTx.issued_qty}`);
      Utils.log(`今日の期末在庫: ${todayOpening + todayTx.received_qty - todayTx.issued_qty} = ${currentQty} (検証)`);
      ...
      if (todaySummary) {
        Utils.log(`今日のサマリー:`, todaySummary);
      }
